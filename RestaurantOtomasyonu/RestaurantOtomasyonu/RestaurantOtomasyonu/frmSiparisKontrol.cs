...
namespace RestaurantOtomasyonu
{
    public partial class frmSiparisKontrol : Form
    {
        ...
        private void frmSiparisKontrol_Load(object sender, EventArgs e)
        {
            ...
            if (butonSayisi > lvMusteriler.Items.Count)
            {
                butonSayisi = lvMusteriler.Items.Count;
            }
        ...
        protected void dinamikMetod2(object sender, EventArgs e)
        {
            ...
            cGenel._ServisTurNo = 2;
            cGenel._AdisyonId = Convert.ToString(c.musterininsonadisyonId(Convert.ToInt32(dinamikButon.Name)));
            lblGenelToplam.Text = s.GenelToplam(Convert.ToInt32(dinamikButon.Name)).ToString() + "TL";
        ...
        void toplam()
        {
            int kayitsayisi = lvMusteriDetaylari.Items.Count;
            decimal toplam = 0;
        ...
        private void lvMusteriDetaylari_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (lvMusteriDetaylari.SelectedItems.Count>0)
            {
                ...
                c.adisyonpaketsiparisDetaylari(lvSatisDetaylari, Convert.ToInt32(lvSatisDetaylari.SelectedItems[0].SubItems[4].Text));
                toplam();

                lblGenelToplam.Text = c.GenelToplam(Convert.ToInt32(lvMusteriDetaylari.SelectedItems[0].SubItems[0].Text)).ToString() + " TL ";
            }
        }
